...
# 1.2. Create Folders
import main
main.check_and_make_directories([config.DATA_SAVE_DIR, config.TRAINED_MODEL_DIR, config.TENSORBOARD_LOG_DIR, config.RESULTS_DIR])
...
dp.run(ticker_list = config_tickers.DOW_30_TICKER, technical_indicator_list = config.INDICATORS, if_vix=False)
df = dp.dataframe
...
for i in range(lookback, len(df.index.unique())):
    ...
    price_lookback = data_lookback.pivot_table(index="date", columns="tic", values="close").dropna(axis=1)
...
baseline_df = get_baseline(ticker='^DJI', start=df_daily_return.loc[0,'date'], end='2021-11-01')
