...
from meta.env_market_impact.envs.env_mace_stock_trading import MACEStockTradingEnv, EnvParams
from meta.env_market_impact.envs.impact_models import SqrtImpactModel, ACImpactModel, BaselineImpactModel, ImpactModel
...
def run_and_log_simulation(env: MACEStockTradingEnv, model, dates: list, benchmark_df: pd.DataFrame, reset_impact_model: bool = True, initial_benchmark_value: float = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    start_value = initial_benchmark_value if initial_benchmark_value is not None else env.initial_capital
...
def train_with_epoch_evaluation(
    ...
    for epoch in range(num_epochs):
        ...
        eval_train_results, eval_train_trades = run_and_log_simulation(
            eval_train_env, model, train_config["date_list"],
            train_benchmark_df, reset_impact_model=True,
        )
        ...
        eval_trade_results, eval_trade_trades = run_and_log_simulation(
            eval_trade_env, model, trade_config["date_list"],
            trade_benchmark_df, reset_impact_model=True,
        )
...
def train_and_backtest(
    ...
    for idx, params in enumerate(backtest_grid, 1):
        ...
        log.info(f"Training for {num_epochs} epochs ({total_timesteps} total timesteps).")

        trained_model, epoch_stats_train, epoch_stats_test_blank = train_with_epoch_evaluation(
            model=model,
            train_env=train_env,
        ...
        train_results_df, train_trades_df = run_and_log_simulation(train_env, trained_model, train_config["date_list"], train_benchmark_df, reset_impact_model=True)
        ...
        trade_results_df, trade_trades_df = run_and_log_simulation(trade_env, trained_model, trade_config["date_list"], trade_benchmark_df, reset_impact_model=False, initial_benchmark_value=last_train_benchmark_value)
        ...
        trade_results_blank_df, trade_blank_trades_df = run_and_log_simulation(trade_env_blank, trained_model, trade_config["date_list"], trade_benchmark_df, reset_impact_model=True)
        test_blank_csv_filename = f"{results_dir}/{base_filename}_test_blank.csv"
        test_blank_trades_csv_filename = f"{results_dir}/{base_filename}_test_blank_trades.csv"
        ...
        all_backtests_metadata.append({
            ...
            "net_arch": str(params.policy_kwargs.get("net_arch")) if params.policy_kwargs else None,
...
def run_example():
    ...
    backtest_grid = BacktestParams.from_explicit(
        configs=[
            ...
            {"model_name": "a2c", "impact_model_class": BaselineImpactModel, "initial_capital": 1e9},
            # A2C | ACImpact | default params
            {"model_name": "a2c", "impact_model_class": ACImpactModel, "initial_capital": 1e9},
            # A2C | Baseline | HPO
            {
                "model_name": "a2c",
                "impact_model_class": BaselineImpactModel,
                "initial_capital": 1e9,
                "model_kwargs": {"learning_rate": 9.345473014207437e-05, "n_steps": 50, "ent_coef": 0.0007879004401207801, "gamma": 0.9088236029155305, "gae_lambda": 0.8085192755608721},
            ...
            {"model_name": "ppo", "impact_model_class": BaselineImpactModel, "initial_capital": 1e9},
            # PPO | ACImpact | default params
            {"model_name": "ppo", "impact_model_class": ACImpactModel, "initial_capital": 1e9},
            # PPO | Baseline | HPO
            {
                "model_name": "ppo",
                "impact_model_class": BaselineImpactModel,
                "initial_capital": 1e9,
                "model_kwargs": {"learning_rate": 1.4157895169765918e-05, "n_steps": 1024, "batch_size": 128, "ent_coef": 0.00017806504001840215, "gamma": 0.9341189226765384, "gae_lambda": 0.9244743605610781, "clip_range": 0.1474194639040638, "n_epochs": 10},
                "policy_kwargs": {"net_arch": [256, 128, 64]},
            ...
            {"model_name": "ddpg", "impact_model_class": BaselineImpactModel, "initial_capital": 1e9},
            # DDPG | ACImpact | default params
            {"model_name": "ddpg", "impact_model_class": ACImpactModel, "initial_capital": 1e9},
            # DDPG | Baseline | HPO
            {
                "model_name": "ddpg",
                "impact_model_class": BaselineImpactModel,
                "initial_capital": 1e9,
                "model_kwargs": {"learning_rate": 0.0014255481008962508, "batch_size": 64, "buffer_size": 50000, "gamma": 0.9667387593404914, "tau": 0.020555160629308206},
                "policy_kwargs": {"net_arch": [256, 128, 64]},
            ...
            {"model_name": "sac", "impact_model_class": BaselineImpactModel, "initial_capital": 1e9},
            # SAC | ACImpact | default params
            {"model_name": "sac", "impact_model_class": ACImpactModel, "initial_capital": 1e9},
            # SAC | Baseline | HPO
            {
                "model_name": "sac",
                "impact_model_class": BaselineImpactModel,
                "initial_capital": 1e9,
                "model_kwargs": {"learning_rate": 0.00011684631812979796, "batch_size": 256, "buffer_size": 10000, "gamma": 0.9989066452742179, "tau": 0.015505359206935673, "ent_coef": "auto"},
            ...
            {"model_name": "td3", "impact_model_class": BaselineImpactModel, "initial_capital": 1e9},
            # TD3 | ACImpact | default params
            {"model_name": "td3", "impact_model_class": ACImpactModel, "initial_capital": 1e9},
            # TD3 | Baseline | HPO
            {
                "model_name": "td3",
                "impact_model_class": BaselineImpactModel,
                "initial_capital": 1e9,
                "model_kwargs": {"learning_rate": 0.00015265277410336577, "batch_size": 128, "buffer_size": 50000, "gamma": 0.974062775740949, "tau": 0.04060258378797207},
                "policy_kwargs": {"net_arch": [256, 128, 64]},
