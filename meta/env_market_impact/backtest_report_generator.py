...
from .envs.utils import get_logger, compute_performance_stats, rolling_sharpe
...
class BacktestReportGenerator:
    ...
    _REPORT_CHARTS = [
        Chart.PORTFOLIO_VALUE, Chart.TURNOVER, Chart.CUMULATIVE_COST,
        Chart.ROLLING_SHARPE, Chart.DRAWDOWN, Chart.TRADING_VOLUME,
        Chart.PORTFOLIO_VALUE_BLANK_SLATE, Chart.ROLLING_SHARPE_BLANK_SLATE,
        Chart.TRADING_VOLUME_BLANK_SLATE, Chart.DRAWDOWN_BLANK_SLATE,
        Chart.POV_HISTOGRAM_TRAIN, Chart.POV_HISTOGRAM_TEST,
        Chart.POV_HISTOGRAM_BLANK_SLATE, Chart.POV_TIME_SERIES,
        Chart.POV_TIME_SERIES_BLANK_SLATE, Chart.EPOCH_STATS,
    ...
    def __init__(self, summary_json_path: str, filters: Optional[dict] = None,
        ...
        if exclude_filters:
            ...
            log.info(f"{len(backtests_metadata)} backtests after exclusion filter ({prev_count - len(backtests_metadata)} excluded)")
        ...
        if display_keys is not None:
            ...
            self._varying_keys: set[str] = self._get_varying_param_keys(backtests_metadata)
        ...
        for i, metadata in enumerate(backtests_metadata):
            df_train, df_test, df_test_blank, trades_train, trades_test, trades_blank = \
            ...
            blank_metrics = self._calculate_performance_metrics(df_test_blank, trades_blank)
        ...
        self._table_headers = [
            ...
            "Total Sharpe Ratio", "Annualized Sharpe Ratio",
            "Ann. Volatility (%)", "Sortino Ratio", "Calmar Ratio",
            "Avg Daily Turnover (%)", "Max Drawdown (%)",
            "Total Trading Cost ($)", "Avg Daily Trading Cost ($)",
            "Avg POV (%)", "Wtd Avg Turnover Pctl", "Avg Step Reward",
        ]
        self._table_data = (
            [[bt['short_name'], "Train"] + bt['train_metrics'] for bt in self._backtests]
            + [[bt['short_name'], "Test"] + bt['test_metrics'] for bt in self._backtests]
            + [[bt['short_name'], "Blank Slate"] + bt['blank_metrics'] for bt in self._backtests]
        )

        log.info(f"Loaded {len(self._backtests)} backtests")


    def make_chart(self, chart: Chart, legend_position: str = "outside",
                   legend_columns: int = 1,
                   height: Optional[int] = None,
                   legend_gap: Optional[float] = None) -> go.Figure:
        """
        ...
        self._apply_legend_position(fig, legend_position, legend_columns,
                                    legend_gap)
        return fig

    def generate_report(self, output_path: str = None,
                        legend_position: str = "outside",
                        legend_columns: int = 1) -> str:
        """
        ...
        for chart in self._REPORT_CHARTS:
            fig = self.make_chart(chart, legend_position=legend_position,
    ...
    def _make_portfolio_value(self) -> go.Figure:
        ...
        for bt in self._backtests:
            ...
            fig.add_trace(go.Scatter(
                x=bt['df_train']['date'], y=bt['df_train']['portfolio_value'],
                mode='lines', name=bt['trace_name'], line=dict(color=tc),
                legendgroup=bt['trace_name'],
            ))
            fig.add_trace(go.Scatter(
                x=bt['df_test']['date'], y=bt['df_test']['portfolio_value'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'], showlegend=False,
            ))
        for bt in self._backtests:
            if bt['add_benchmark']:
    ...
    def _make_turnover(self) -> go.Figure:
        ...
        for bt in self._backtests:
            tc, ttc = self._train_test_colors(bt['rgb'])
            fig.add_trace(go.Scatter(
                x=bt['df_train']['date'], y=bt['df_train']['turnover'] * 100,
                mode='lines', name=bt['trace_name'], line=dict(color=tc),
                legendgroup=bt['trace_name'],
            ))
            fig.add_trace(go.Scatter(
                x=bt['df_test']['date'], y=bt['df_test']['turnover'] * 100,
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'], showlegend=False,
            ))
        self._add_split_vline(fig)
    ...
    def _make_cumulative_cost(self) -> go.Figure:
        ...
        for bt in self._backtests:
            tc, ttc = self._train_test_colors(bt['rgb'])
            fig.add_trace(go.Scatter(
                x=bt['df_train']['date'], y=bt['df_train']['cumulative_cost'],
                mode='lines', name=bt['trace_name'], line=dict(color=tc),
                legendgroup=bt['trace_name'],
            ))
            fig.add_trace(go.Scatter(
                x=bt['df_test']['date'], y=bt['df_test']['cumulative_cost'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'], showlegend=False,
            ))
        self._add_split_vline(fig)
    ...
    def _make_rolling_sharpe(self) -> go.Figure:
        ...
        for bt in self._backtests:
            tc, ttc = self._train_test_colors(bt['rgb'])
            fig.add_trace(go.Scatter(
                x=bt['df_train']['date'], y=bt['df_train']['rolling_sharpe'],
                mode='lines', name=bt['trace_name'], line=dict(color=tc),
                legendgroup=bt['trace_name'],
            ))
            fig.add_trace(go.Scatter(
                x=bt['df_test']['date'], y=bt['df_test']['rolling_sharpe'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'], showlegend=False,
            ))
        for bt in self._backtests:
    ...
    def _make_drawdown(self) -> go.Figure:
        ...
        for bt in self._backtests:
            tc, ttc = self._train_test_colors(bt['rgb'])
            fig.add_trace(go.Scatter(
                x=bt['df_train']['date'], y=bt['df_train']['drawdown'],
                mode='lines', name=bt['trace_name'], line=dict(color=tc),
                legendgroup=bt['trace_name'],
            ))
            fig.add_trace(go.Scatter(
                x=bt['df_test']['date'], y=bt['df_test']['drawdown'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'], showlegend=False,
            ))
        self._add_split_vline(fig)
    ...
    def _make_trading_volume(self) -> go.Figure:
        ...
        for bt in self._backtests:
            ...
            fig.add_trace(go.Bar(
                x=bt['df_train']['date'], y=bt['df_train']['total_buy_value'],
                name=f"{grp} - Buys", marker_color=tc,
                legendgroup=grp, offsetgroup=og,
            ))
            fig.add_trace(go.Bar(
                x=bt['df_train']['date'], y=-bt['df_train']['total_sell_value'],
                name=f"{grp} - Sells", marker_color=sc_train,
                legendgroup=grp, offsetgroup=og, showlegend=False,
            ))
            fig.add_trace(go.Bar(
                x=bt['df_test']['date'], y=bt['df_test']['total_buy_value'],
                name=grp, marker_color=ttc,
                legendgroup=grp, offsetgroup=og, showlegend=False,
            ))
            fig.add_trace(go.Bar(
                x=bt['df_test']['date'], y=-bt['df_test']['total_sell_value'],
                name=grp, marker_color=sc_test,
                legendgroup=grp, offsetgroup=og, showlegend=False,
            ))

            train_cash_pct = bt['df_train']['cash'] / bt['df_train']['portfolio_value'] * 100
            test_cash_pct = bt['df_test']['cash'] / bt['df_test']['portfolio_value'] * 100
            fig.add_trace(go.Scatter(
                x=bt['df_train']['date'], y=train_cash_pct, mode='lines',
                name=cash_grp, line=dict(color=gc_train),
                legendgroup=cash_grp,
            ), secondary_y=True)
            fig.add_trace(go.Scatter(
                x=bt['df_test']['date'], y=test_cash_pct, mode='lines',
                name=cash_grp, line=dict(color=gc_test),
                legendgroup=cash_grp, showlegend=False,
            ), secondary_y=True)
    ...
    def _make_portfolio_value_blank_slate(self) -> go.Figure:
        ...
        for bt in self._backtests:
            ...
            fig.add_trace(go.Scatter(
                x=bt['df_test_blank']['date'], y=bt['df_test_blank']['portfolio_value'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'],
            ))
        for bt in self._backtests:
    ...
    def _make_rolling_sharpe_blank_slate(self) -> go.Figure:
        ...
        for bt in self._backtests:
            _, ttc = self._train_test_colors(bt['rgb'])
            fig.add_trace(go.Scatter(
                x=bt['df_test_blank']['date'], y=bt['df_test_blank']['rolling_sharpe'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'],
            ))
        for bt in self._backtests:
    ...
    def _make_trading_volume_blank_slate(self) -> go.Figure:
        ...
        for bt in self._backtests:
            ...
            fig.add_trace(go.Bar(
                x=bt['df_test_blank']['date'], y=bt['df_test_blank']['total_buy_value'],
                name=f"{grp} - Buys", marker_color=ttc,
                legendgroup=grp, offsetgroup=og,
            ))
            fig.add_trace(go.Bar(
                x=bt['df_test_blank']['date'], y=-bt['df_test_blank']['total_sell_value'],
                name=f"{grp} - Sells", marker_color=sc,
                legendgroup=grp, offsetgroup=og, showlegend=False,
            ))

            blank_cash_pct = bt['df_test_blank']['cash'] / bt['df_test_blank']['portfolio_value'] * 100
            fig.add_trace(go.Scatter(
                x=bt['df_test_blank']['date'], y=blank_cash_pct, mode='lines',
                name=cash_grp, line=dict(color=gc),
                legendgroup=cash_grp,
            ), secondary_y=True)
    ...
    def _make_drawdown_blank_slate(self) -> go.Figure:
        ...
        for bt in self._backtests:
            _, ttc = self._train_test_colors(bt['rgb'])
            fig.add_trace(go.Scatter(
                x=bt['df_test_blank']['date'], y=bt['df_test_blank']['drawdown'],
                mode='lines', name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'],
            ))
        fig.update_layout(
    ...
    def _make_pov_histogram(self, period: str) -> go.Figure:
        ...
        trades_key = {'train': 'trades_train', 'test': 'trades_test',
                      'blank_slate': 'trades_blank'}[period]
        title_period = {'train': 'Train', 'test': 'Test',
                        'blank_slate': 'Blank Slate'}[period]

        fig = make_subplots(
            rows=1, cols=2,
            subplot_titles=(f"POV Distribution ({title_period})",
        ...
        trades_list = [(bt[trades_key], bt['index'], bt['trace_name'])
                       for bt in self._backtests if not bt[trades_key].empty]
        n = len(trades_list)
        if n == 0:
            fig.update_layout(
        ...
        for j, (trades_df, bt_idx, trace_name) in enumerate(trades_list):
            ...
            hw, be = np.histogram(trades_df['pov'] * 100, bins=pov_bins, weights=notional)
            centers = (be[:-1] + be[1:]) / 2
            fig.add_trace(go.Bar(
                x=centers + pov_offset, y=hw, width=pov_bw,
                name=trace_name, marker_color=color,
            ...
            hw, be = np.histogram(trades_df['turnover_percentile'],
                                  bins=pctl_bins, weights=notional)
            centers = (be[:-1] + be[1:]) / 2
            fig.add_trace(go.Bar(
                x=centers + pctl_offset, y=hw, width=pctl_bw,
                name=trace_name, marker_color=color,
    ...
    def _make_pov_time_series(self) -> go.Figure:
        ...
        for bt in self._backtests:
            tc, ttc = self._train_test_colors(bt['rgb'])
            shown = False
            for trades_df, color in [(bt['trades_train'], tc),
                                     (bt['trades_test'], ttc)]:
                if trades_df.empty:
                    continue
                daily_pov = self._daily_avg_pov(trades_df)
                fig.add_trace(go.Scatter(
                    x=daily_pov['date'], y=daily_pov['avg_pov'], mode='lines',
                    name=bt['trace_name'], line=dict(color=color),
                    legendgroup=bt['trace_name'], showlegend=not shown,
                ))
                shown = True
        self._add_split_vline(fig)
        fig.update_layout(
    ...
    def _make_pov_time_series_blank_slate(self) -> go.Figure:
        ...
        for bt in self._backtests:
            ...
            fig.add_trace(go.Scatter(
                x=daily_pov['date'], y=daily_pov['avg_pov'], mode='lines',
                name=bt['trace_name'], line=dict(color=ttc),
                legendgroup=bt['trace_name'],
            ))
        fig.update_layout(
    ...
    _EPOCH_STAT_CONFIGS = [
        ...
        ("annualized_volatility",   "Annualized Volatility per Epoch (%)",       "Ann. Volatility (%)"),
        ...
        ("avg_daily_trading_cost",  "Avg Daily Trading Cost per Epoch ($)",      "Avg Cost ($)"),
            ...
            for bt in self._backtests:
                ...
                if train_stats:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        line=dict(color=tc), marker=dict(size=5),
                        legendgroup=bt['trace_name'],
    ...
    def _make_single_epoch_stat(self, stat_index: int) -> go.Figure:
        ...
        fig.update_layout(
            ...
            height=400, hoverlabel=dict(namelength=-1),
        )
        return fig

    def _make_epoch_annualized_return(self):  return self._make_single_epoch_stat(0)
    def _make_epoch_annualized_sharpe(self):  return self._make_single_epoch_stat(1)
    def _make_epoch_annualized_volatility(self): return self._make_single_epoch_stat(2)
    def _make_epoch_sortino(self):            return self._make_single_epoch_stat(3)
    def _make_epoch_calmar(self):             return self._make_single_epoch_stat(4)
    def _make_epoch_max_drawdown(self):       return self._make_single_epoch_stat(5)
    def _make_epoch_avg_turnover(self):       return self._make_single_epoch_stat(6)
    def _make_epoch_avg_cost(self):           return self._make_single_epoch_stat(7)
    def _make_epoch_avg_pov(self):            return self._make_single_epoch_stat(8)
    def _make_epoch_avg_reward(self):         return self._make_single_epoch_stat(9)
    ...
    def _make_performance_table(self) -> go.Figure:
        ...
        fig.add_trace(go.Table(
            header=dict(
                ...
                font=dict(size=12), line=dict(color='#cccccc', width=1),
            ),
            cells=dict(
                values=table_values,
    ...
    def _add_benchmark_scatter(self, fig, bt: dict, column: str):
        ...
        fig.add_trace(go.Scatter(
            x=bt['df_train']['date'], y=bt['df_train'][column],
            mode='lines', name=label, line=dict(color='rgba(0, 0, 0, 0.7)'),
            legendgroup=label,
        ))
        fig.add_trace(go.Scatter(
            x=bt['df_test']['date'], y=bt['df_test'][column],
            mode='lines', name=label, line=dict(color='rgb(0, 0, 0)'),
    ...
    def _add_benchmark_scatter_blank(self, fig, bt: dict, column: str):
        ...
        label = f"{self._benchmark_ticker} | Capital={capital_str}"
        fig.add_trace(go.Scatter(
            x=bt['df_test_blank']['date'], y=bt['df_test_blank'][column],
            mode='lines', name=label, line=dict(color='rgb(0, 0, 0)'),
            legendgroup=label,
        ))
    ...
    def _add_split_vline(self, fig):
        ...
        if self._split_date:
            fig.add_vline(
                x=self._split_date,
                line_width=2, line_dash="dash", line_color="grey",
            )

    @staticmethod
    def _apply_legend_position(fig: go.Figure, position: str,
                               columns: int = 1,
        ...
        if position == "inside":
            ...
            legend_opts = dict(
                x=0.0, y=-gap, xanchor='left', yanchor='top',
                font=dict(size=10),
                tracegroupgap=2,
            )
            if columns > 1:
    ...
    def _daily_avg_pov(trades_df: pd.DataFrame) -> pd.DataFrame:
        ...
        daily = trades_df.groupby('date').apply(
            lambda x: (x['pov'] * x['notional']).sum() / x['notional'].sum() * 100
            if x['notional'].sum() > 0 else 0
        ).reset_index()
    ...
    def _load_and_prepare_data(self, train_csv_path: str, test_csv_path: str, test_blank_csv_path: str,
                                trades_train_path: str = None, trades_test_path: str = None, trades_blank_path: str = None):
        ...
        trades_train = pd.read_csv(trades_train_path) if trades_train_path and os.path.exists(trades_train_path) else pd.DataFrame()
        trades_test = pd.read_csv(trades_test_path) if trades_test_path and os.path.exists(trades_test_path) else pd.DataFrame()
        trades_blank = pd.read_csv(trades_blank_path) if trades_blank_path and os.path.exists(trades_blank_path) else pd.DataFrame()
        ...
        if not df_test.empty:
            ...
            df_test_blank = df_test_blank[df_test_blank['date'] >= test_start_date].copy()
        ...
        df_combined["benchmark_rolling_sharpe"] = rolling_sharpe(df_combined["benchmark_daily_return"])
        ...
        df_train["benchmark_rolling_sharpe"] = df_combined[train_mask]["benchmark_rolling_sharpe"].values
        df_test["benchmark_rolling_sharpe"] = df_combined[~train_mask]["benchmark_rolling_sharpe"].values
        ...
        df_test_blank["benchmark_daily_return"] = df_test_blank["benchmark_value"].pct_change()

        rolling_max_blank = df_test_blank['portfolio_value'].expanding(min_periods=1).max()
        df_test_blank['drawdown'] = (df_test_blank['portfolio_value'] - rolling_max_blank) / rolling_max_blank * 100

        df_test_blank["rolling_sharpe"] = rolling_sharpe(df_test_blank["daily_return"], window=63, min_periods=21)
        df_test_blank["benchmark_rolling_sharpe"] = rolling_sharpe(df_test_blank["benchmark_daily_return"], window=63, min_periods=21)
    ...
    def _calculate_performance_metrics(self, df, trades_df=None):
        ...
        stats = compute_performance_stats(
            df["portfolio_value"], df["turnover"], df["cost"], trades_df,
    ...
    _KEY_PRIORITY = [
        'run_type', 'drl_agent', 'impact_model', 'initial_capital',
        'eta_dd', 'horizon', 'reward_scaling',
        'with_tbill', 'with_perm', 'with_cooldown', 'use_obs_normalizer', 'obs_clip',
        'learning_rate', 'gamma', 'ent_coef', 'net_arch',
    ]
    ...
    def _build_trace_name(self, metadata: dict, varying_keys: set[str]) -> str:
        ...
        add('Ent', 'ent_coef', lambda x: f"{float(x):.3f}" if isinstance(x, (int, float)) else str(x))
        add('Arch', 'net_arch', str)

        if not parts and 'drl_agent' in metadata:
            parts.append(f"Agent={str(metadata['drl_agent']).upper()}")

        return self._truncate_parts(parts, " | ", self._MAX_TRACE_NAME_LEN) or "Backtest"
    ...
    def _build_short_name(self, metadata: dict, varying_keys: set[str]) -> str:
        ...
        add('ent_coef', lambda x: f"e={float(x):.3f}" if isinstance(x, (int, float)) else f"e={x}")
        add('net_arch', lambda x: f"a={x}")

        return self._truncate_parts(parts, "-", self._MAX_SHORT_NAME_LEN) or str(metadata.get('drl_agent', 'Backtest')).upper()
